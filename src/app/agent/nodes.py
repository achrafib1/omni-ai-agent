...
from src.app.agent.utils.llm_factory import get_core_reasoning_model
...
class MemoryExtractionResponse(BaseModel):
    ...
    is_important: bool = Field(
        description="True if the message contains a personal fact to remember."
    )
    formatted_memory: str = Field(
        default="", description="The isolated fact formatted in third person."
    )
...
async def memory_extraction_node(state: OmniAgentState) -> Dict[str, Any]:
    ...
    extraction_llm = get_core_reasoning_model(temperature=0.1).with_structured_output(
    ...
    try:
        analysis = cast(
            MemoryExtractionResponse, await extraction_llm.ainvoke([HumanMessage(content=prompt)])
        )
        if analysis.is_important and analysis.formatted_memory:
            logger.info(
                f"Fact detected: '{analysis.formatted_memory}'. Dispatching to MCP Network..."
            )
...
async def router_node(state: OmniAgentState) -> Dict[str, Any]:
    ...
    try:
        response = cast(
            RouterResponse,
            await router_llm.ainvoke([SystemMessage(content=routing_prompt), *messages_to_analyze]),
        )
...
async def memory_injection_node(state: OmniAgentState) -> Dict[str, Any]:
    ...
    memories = await mcp_client.execute_tool(
        tool_name="retrieve_user_memories", arguments={"user_id": user_id, "query": latest_query}
    )
