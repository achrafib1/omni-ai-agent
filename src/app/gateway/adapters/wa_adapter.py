...
class WhatsAppAdapter(BaseChannelAdapter):
    ...
    async def _download_media(self, media_id: str) -> Optional[bytes]:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            try:
                ...
                if not media_url:
                    logger.error(
                        f"[danger]Meta API did not return a URL for media {media_id}.[/danger]"
                    )
                    return None
                ...
                logger.info(
                    f"[success]Successfully downloaded media {media_id} ({len(download_res.content)} bytes).[/success]"
                )
                return download_res.content

            except httpx.HTTPStatusError as e:
                logger.error(
                    f"[danger]HTTP Error downloading WhatsApp media:[/danger] {e.response.text}"
                )
                return None
            except Exception as e:
                logger.error(
                    f"[danger]Unexpected error downloading WhatsApp media:[/danger] {e}",
                    exc_info=True,
                )
                return None
    ...
    async def parse_http_payload(self, payload: Dict[str, Any]) -> Optional[OmniMessage]:
        ...
        try:
            ...
            logger.error(
                f"[danger]Malformed WhatsApp payload encountered:[/danger] {e}", exc_info=True
            )
            return None
    ...
    async def send_http_message(self, recipient_id: str, text: str) -> bool:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                logger.error(
                    f"[danger]Failed to send WhatsApp message:[/danger] HTTP {e.response.status_code} - {e.response.text}"
                )
                return False
            except Exception as e:
                logger.error(
                    f"[danger]Unexpected error dispatching WA message:[/danger] {e}", exc_info=True
                )
                return False
