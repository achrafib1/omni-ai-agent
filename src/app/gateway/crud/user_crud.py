...
async def get_or_create_user(
    ...
    """
    logger.debug(f"Querying database for user on platform '{platform.value}' with ID '{platform_user_id}'.")
    ...
    logger.info(f"[info]New user detected on '{platform.value}'. Provisioning database record.[/info]")
    new_user = User(
    ...
    try:
        ...
        logger.info(f"[success]Successfully created new user. Internal UUID: {new_user.id}[/success]")
        return new_user
    except IntegrityError as e:
        # Handle concurrent race conditions where the user was created milliseconds ago
        await db.rollback()
        logger.warning(f"IntegrityError during user creation. Attempting to fetch existing record: {e}")
