...
class MessageBus:
    ...
    async def handle_http_webhook(
        self, adapter: BaseChannelAdapter, payload: Dict[str, Any]
    ) -> bool:
        """
        ...
        async with AsyncSessionLocal() as db_session:
            user_record = await get_or_create_user(
                db_session, omni_msg.platform, omni_msg.platform_user_id
            )
        ...
        opik_tracer = OpikTracer(
            project_name=settings.OPIK_PROJECT_NAME, tags=["stream_execution", platform_name]
        )

        config = {"configurable": {"thread_id": omni_msg.session_id}, "callbacks": [opik_tracer]}

        # 4. Execute LangGraph to completion
        final_message = "I am processing a heavy cognitive load. Could you repeat that?"
        try:
            logger.debug(
                f"[{platform_name}] Awaiting LangGraph resolution for session {omni_msg.session_id}"
            )
            async for state_update in self.agent.astream(
            ...
            logger.error(
                f"[danger]LangGraph execution failed during Sync Webhook:[/danger] {e}",
                exc_info=True,
            )
            return False

    async def handle_websocket_stream(
        self, adapter: BaseChannelAdapter, websocket: WebSocket
    ) -> None:
        """
        ...
        logger.info(
            f"[{platform_name}] WebSocket connection established. Thread assigned: {session_id}"
        )

        try:
            await websocket.send_json(
                {"type": "status", "content": "Neural link established. Ready."}
            )
            ...
            while True:
                ...
                opik_tracer = OpikTracer(
                    project_name=settings.OPIK_PROJECT_NAME, tags=["sync_execution", platform_name]
                )
                ...
                async for token in self.agent.astream(
                    ...
                    if isinstance(token, str) and token:
                        await websocket.send_json(
                            {"type": "token", "content": token, "is_final": False}
                        )

                # Signal completion of this turn
                await websocket.send_json({"type": "token", "content": "", "is_final": True})

        except WebSocketDisconnect:
            logger.info(
                f"[{platform_name}] WebSocket client disconnected. (Session: {session_id})."
            )
        except Exception as e:
            logger.error(f"[{platform_name}] WebSocket exception:[/danger] {e}", exc_info=True)
            with contextlib.suppress(Exception):
                await websocket.send_json(
                    {"type": "error", "content": "Internal cognitive failure."}
                )
