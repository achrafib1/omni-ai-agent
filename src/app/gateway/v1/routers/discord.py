...
from fastapi import APIRouter, BackgroundTasks, Depends, Request, WebSocket, status
...
async def discord_webhook(
    ...
    background_tasks.add_task(
        message_bus.handle_http_webhook, adapter=discord_adapter, payload=payload
    )
    return {"status": "processing"}


@router.websocket("/stream")
async def discord_stream(websocket: WebSocket, message_bus: MessageBus = Depends(get_message_bus)):
    """
    ...
    """
    await message_bus.handle_websocket_stream(adapter=discord_adapter, websocket=websocket)
