...
from fastapi import APIRouter, Depends, Request, WebSocket, BackgroundTasks, Query, HTTPException, status
...
async def verify_whatsapp_webhook(
    ...
    expected_token = verify_token.get_secret_value() if hasattr(verify_token, "get_secret_value") else str(verify_token)
...
async def receive_whatsapp_webhook(
    ...
    try:
        ...
        logger.error(f"[danger]Failed to read WhatsApp Webhook payload:[/danger] {e}", exc_info=True)
