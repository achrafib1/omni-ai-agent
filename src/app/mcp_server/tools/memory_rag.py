...
try:
    ...
    embedder = GoogleGenerativeAIEmbeddings(
        model="gemini-embedding-2", output_dimensionality=768, api_key=settings.GEMINI_API_KEY
    )
...
async def retrieve_user_memories(user_id: str, query: str) -> str:
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            logger.info(
                f"[success]Retrieved {len(memories)} semantic memories for User {user_id}[/success]"
            )
